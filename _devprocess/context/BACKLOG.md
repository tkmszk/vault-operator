...
Last update: 2026-05-20 by coding (EPIC-29 Welle 1 Implementation abgeschlossen: FEAT-29-01 alle 11 PLAN-27-Tasks gruen. agentFolder-Helper (status-aware), migrateAgentLayout-Service (10 Phasen, idempotent), useVaultLocalRoot in GlobalFileService, Wayfinder gepflegt. Restore-Service als Sicherheitsnetz (Backup-Snapshot in {vault}/.obsidian/plugins/<id>/layout-migration-backups/), drei Settings-Reset-Buttons (Activate / Reset to default / Restore from backup), ChatHistoryFolderRemovedModal als one-shot. UI komplett auf Englisch ohne Feature-IDs. Live-Test auf 288 MB knowledge.db / iCloud-Vault erfolgreich. Build green, 51 Tests gruen (Restore + Migration + agentFolder). Naechster Schritt: /testing -> /security-audit, dann Welle 2 (FEAT-29-02 Plugin-Skill-Format-Migration).)
