...
| FIX-04-03-07 | Fix | 03-07: DeepSeek deepseek-reasoner 400 -- reasoning_content not echoed back on assistant turns with tool_calls | Active | Building | FEAT-04-03, EPIC-04 | BUG |  | sebastian-claude-opus-4-7 | 2026-05-28 | P1 Issue [#38](https://github.com/pssah4/vault-operator/issues/38). Root cause: deepseek-reasoner liefert reasoning_content + content; bei Assistant-Message mit tool_calls erwartet die DeepSeek-API den reasoning_content auf dem naechsten Request, wir verwerfen den Stream-Text aber in AgentTask. **Fix:** ThinkingBlock als first-class ContentBlock, requiresPassback-Flag auf Stream-Chunk gated auf OpenAI-kompatibel + Allow-List {custom, ollama, lmstudio}, last-assistant-only-Echo schuetzt vor Token-Explosion, 50k-Cap, Empty-Guard. Anthropic + Bedrock kriegen defensive Pre-Filter gegen Cross-Provider-Switch-Crash. UI-Replay via reasoningText? auf UiMessage. OpenRouter + Anthropic ET+TU bleiben out of scope (separater Bug). Spec: `_devprocess/requirements/fixes/FIX-04-03-07-deepseek-reasoning-passback.md` |
