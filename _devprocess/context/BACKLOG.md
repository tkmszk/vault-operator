...
Last update: 2026-05-21 by coding (FEAT-29-14 Phase 2: Templates-Setup. Bundled-Templates auf `bundled-templates/notes/{de,en}/` segmentiert, esbuild-Generator emittiert `BUNDLED_NOTE_TEMPLATES`. Neuer `resolveCoreTemplatesFolder`-Helper (liest Obsidian-Core-Templates folder aus `templates.json`). Neuer `TemplateMaterializer` (skip-existing/force, LLM-Translator-Callback fuer Sprachen != DE/EN, EN-Fallback wenn kein Modell aktiv). FirstRunWizardModal um 'templates'-Step erweitert: Sprach-Dropdown (DE/EN/Other), auto-detected Templates-Folder, Materialize-Toggle. VaultTab: `quellenNotizTemplate`-Field + Re-materialize-Button mit Confirm. Settings erweitert um `quellenNotizTemplate` + `templatesLanguage`. 15/15 neue Tests gruen. Plan: `~/.claude/plans/radiant-snacking-piglet.md`. Phase 3 (Ingest-Skill-Updates fuer Sense-Making + Backlink) folgt als FEAT-29-15 auf separatem Branch. FEAT-29-13 vorab: skill-source-Labels (Built-in/Agent/User) released on separate branch.)

[Previous]: Last update: 2026-05-21 by coding (FEAT-29-05 Skill-Creator-Builtin done. Bundle skill-creator builtin (SKILL.md + 2 JS scripts + 2 references) under bundled-skills/, materialized to data/skills/skill-creator/ on plugin onload. manage_skill-Tool entfernt komplett (Wiring + ToolName-Union + toolMetadata + AgentSidebarView + System-Prompts + builtinModes). SkillFrontmatterValidator als hard-gate beim Discovery, 16 Tests. TaskRouter eskaliert "build me a skill"-Phrasen auf Flagship/Main-Model. Sandbox-Bridge erweitert: ctx.vault.mkdir + hidden-path support fuer .vault-operator/, EditFileTool adapter-fallback. Validator-Allowlist erweitert (version, keywords, author, when-to-use, argument-hint, context). VaultDNAScanner kebab-case fuer plugin-id im name-Frontmatter (note_uid_generator nicht mehr rejected). UI-polish: user-message-bubble color-mix 20%, customize-tab toolbox-icon. Reranker ONNX-Backend-Fix via globalThis-Symbol.for(onnxruntime) prepop (umgeht IS_NODE_ENV-Trap). Live-Test bestaetigt: weekly-meeting-summary live-erstellt + valide. Konsole-Warnings auf debug heruntergestuft (Ollama ECONNREFUSED, corrupted XLSX). 1918/1939 Tests gruen, kein new regression. Naechster Schritt: /testing + /security-audit fuer FEAT-29-05. Offen in EPIC-29 nach 29-05: FEAT-29-07/08/09/10/12. Wiedervorlage: Reranker-Install-Asset (2.11.7-assets release fehlt).)
