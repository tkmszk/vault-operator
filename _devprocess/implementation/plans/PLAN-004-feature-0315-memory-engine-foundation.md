...
related:
  ...
  - PLAN-003-feature-0314-knowledge-db-hardening.md
---

# PLAN-004 -- Memory v2 Phase 1 Engine Foundation
...
- **ADR-077** legt das Schema fest (9 neue Tabellen, additiv).
- **ADR-080** definiert das Persistenz-Service-Pattern: Stores erhalten DB-Zugriff via Constructor, kennen weder Plugin noch Vault.
- **ADR-082** Topic-Inference: Centroid-basiert mit Lazy-Refresh.
- **ADR-083** Single-Call-Tool-Output-Schema (kommt in Phase 4, hier nur reservieren).
- **ADR-084** Engine-API-Versioning: API ab Phase 1 versioniert via `MEMORY_ENGINE_API_VERSION`.
- **ADR-085** Soft-Delete-Cascade: `deprecated_at` markiert, kein DELETE auf facts.
- **ADR-086** Inference-Pass-Architecture (kommt in Phase 4).
- **ADR-087** Vault-Note-Memory-Source-Pipeline (kommt in Phase 5/6).
- **ADR-062** KV-Cache-Layout: stabile Sections zuerst, DateTime zuletzt.

## Aufgaben (9 Sub-Schritte, jede einzeln deploybar)

### Aufgabe 1 -- MemoryDB Schema v1 -> v2 additiv

**Ziel:** 9 neue Tabellen aus ADR-077 in `memory.db` anlegen, Schema-Versionsverwaltung einfuehren, idempotente Migration.

**Anpassungen:**

- `MEMORY_SCHEMA` in [MemoryDB.ts](../../src/core/knowledge/MemoryDB.ts) erweitern um Tabellen `facts`, `memory_source_notes`, `fact_embeddings`, `fact_edges`, `communication_styles`, `conversation_threads`, `thread_sessions`, `known_topics`, `memory_audit` plus alle Indizes aus ADR-077.
...
- CHECK-Constraint aus ADR-077 wird in JS auch validiert (defensive).
...
### Aufgabe 6 -- ADR-062 KV-Cache-Layout im SystemPromptBuilder
...
- HISTORY_SCHEMA mit `history_chunks` (Spalten gemaess ADR-087, in Phase 6 befuellt -- jetzt nur DDL).
...
## Verifikation (Akzeptanzkriterien aus FEATURE-0315)
...
7. **ADR-062:** SystemPromptBuilder-Test bestaetigt DateTime ausserhalb Cache-Prefix
