...
related:
  ...
  - _devprocess/architecture/ADR-76-episode-fact-boundary.md
  - _devprocess/architecture/ADR-77-memory-v2-storage-schema.md
  - _devprocess/architecture/ADR-78-uri-versioning-schema.md
  - _devprocess/architecture/ADR-79-knowledge-db-hardening.md
supersedes: ADR-13-memory-architecture (im Verlauf der Implementierung)
---

# PLAN-01 -- Memory v2 Master Plan (Pfad alpha)
...
- ADR-62 (KV-Cache-Layout) ist nicht im Code umgesetzt, ohne Fix verbrennt v2 die Cache-Vorteile
...
|---|---|---|---|---|---|
| **0** | Spikes + ADRs | 1.5 Wo | Branch existiert | 3 Spikes (ATTACH+CTE-Performance, FTS5/JSON1-WASM-Bundle-Size, Single-Call-Token-Profil), ADR-76 (Episode-Fact-Boundary), ADR-77 (Storage-Schema mit URI), ADR-78 (URI-Versioning), ADR-79 (Knowledge-DB-Haertung), ADR-62-Implementation-Spec (KV-Cache-Layout) | Engine-API-Sketch fliesst in alle ADRs |
...
| FTS5 + JSON1 in sql.js verfuegbar | Standard-Build hat keines | Custom-WASM-Build oder JS-Layer-Fallback (ADR-77) |
| BUG-012 reicht "Transactions, audit log, periodic backup" | sql.js export ist Full-Blob, nicht atomar | Multi-File-Atomic-Commit zwingend (ADR-79) |
...
| ADR-62 KV-Cache-Layout existiert | Nur architektonisch, nicht im Code | Phase 1 muss Code-Reorder vor v2-Inject |
...
| 6 Markdown-Dateien als Migrationsquelle | Session-Summaries sind schon DB-first (ADR-60) | Differenzierte Migration pro Datei |
| Recipe-Promotion (ADR-58) ignoriert | Aktiv im Code, nutzt Episodes | ADR-76 klaert Episode-Fact-Boundary |
...
| Source-Doc-Plan | Pfad alpha (PLAN-01) |
|---|---|
...
- ADR-78 erweitert um Source-Adapter-Registry (siehe ADR)
- FEAT-03-14: Vault-Rename-Cascade ist nur Vault-internal, externe Schemata akzeptieren stale Refs (dokumentiert)
- FEAT-03-15: Engine-Public-API exportiert `SourceAdapter`-Interface und `AdapterRegistry`-Service
- FEAT-03-17: UnifiedGraphService nutzt AdapterRegistry, KnowledgeGraphAdapter wird einer von vielen
- FEAT-03-18: Mention-Detection erweitert auf alle Schemata (nicht nur Wiki-Links), generisches `mentions: [{uri, label?}]` im Single-Call-Output
- FEAT-03-19: `source_interface`-Spalte erfasst auch externe Quellen (file-system, cloud-provider, web)
...
|---|---|---|---|---|
| **A. Single-Device** | Plugin selbst | Plugin-lokal | Plugin-lokal **oder** Vault-resident (heute konfigurierbar via FEAT-03-01 `obsidian-sync`-Mode) | 1 (dieses Plugin) |
...
**knowledge.db-Lokalitaet ist orthogonal zur Persistenz-Service-Klasse.** Plugin-FEAT-03-01 hat heute schon ein Setting fuer Vault-resident vs. Plugin-lokal. Das bleibt unabhaengig konfigurierbar -- knowledge.db wird in jedem Setup vom Plugin bedient (Vault-Indexer ist Plugin-Pflichtfunktion).
...
**Plugin-MCP bekommt zwei zusaetzliche Tools** in FEAT-03-17 (heute fehlend): `get_vault_implicit_edges(notePath)` und `get_vault_note_metadata(notePath)`. `semantic_search` existiert bereits.
...
Mobile-Support (iOS/iPadOS via Obsidian Mobile, BA-23 referenziert) ist post-MVP, aber Architektur muss Mobile-tauglich bleiben. Folgende Prinzipien werden eingehalten:
...
Diese Architektur-Constraints werden bei jedem Memory-v2-Feature-PR gegen die Spike-Ergebnisse abgeglichen. Re-Triage von BA-23 nach Memory-v2-Phase-3.
...
**Migrations-Wizard-UI (FEAT-03-19-DoD ergaenzt):** Settings-Modal "Memory & UCM" mit Wizard-Schritten 1-7 (Validation, Backup, Plan-Vorschau, Konflikt-Wahl, Progress, Smoke-Test, Revert-Hinweis).

**Konflikt-Resolution bei Reverse-Wechsel:** FactIntegrator aus FEAT-03-18 wird als Merge-Strategie wiederverwendet. Importierte Facts laufen durch denselben Lazy-Conflict-Pfad wie reale Conversation-Inserts. Damit ist die Konflikt-Resolution beim Setup-Wechsel kein Sondercode, sondern existierender Pfad.
...
Die 8 Phasen werden 8 Features unter EPIC-03 (siehe Backlog-Update). Numerierung: FEAT-03-14 bis FEAT-03-21. Detail-Specs entstehen in `/requirements-engineering`.
