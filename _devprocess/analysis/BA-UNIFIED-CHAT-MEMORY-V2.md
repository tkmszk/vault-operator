...
related:
  ...
  - _devprocess/implementation/plans/PLAN-001-memory-v2-master.md
---
...
|---|---|---|---|---|
| **A. Single-Device** | im Plugin selbst | Plugin-lokal in `.obsidian-agent/` | Plugin-lokal **oder** Vault-resident (heute schon konfigurierbar via `obsidian-sync`-Mode in FEATURE-0301) | nur dieses Plugin (1 Worker) |
...
- **Merge** (Default): FactIntegrator (FEATURE-0318) fuehrt Conflict-Resolution wie bei normalem Insert
...
**Crash-Resilienz:** Multi-File-Atomic-Commit-Journal aus ADR-079 wird um Migration-Status erweitert. Bei Plugin-Restart: Recovery-Replay oder Rollback. Quelle bleibt als `.bak` erhalten.
...
- **Anthropic Contextual Retrieval Pattern:** Obsilo ADR-051
