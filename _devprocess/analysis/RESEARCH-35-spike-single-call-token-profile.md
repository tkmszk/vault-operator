...
status: Provisional Green (formaler Test deferred zu Phase 4 / FEAT-03-18)
date: 2026-04-27
related:
...
Klaeren: Bleibt Single-Call-Extraction nach ADR-83-Schema mit Claude Haiku 4.5 unter dem Mem0-Benchmark-Ziel von ~1500 Tokens pro Memory-Operation?

## Methodik (Approximation, ohne echten API-Call)

User-Entscheidung: echter Test deferred zu Phase 4 (FEAT-03-18), wo Single-Call-Pipeline ohnehin implementiert wird. Approximation aus drei Datenpunkten:
...
### Datenpunkt 2: ADR-83-Schema-Schaetzung
...
| **Total p95** | **~10000** | Lang, aber durch Cost-Cap (FEAT-03-18) abgesichert |
...
Token-Cost-Cap (FEAT-03-18 C5) Default 1M Input + 200K Output = $2/Tag. Decken 100x typische Use ab. Bei runaway-Bug greift der Cap.
...
- Begruendung: ADR-83-Schema ist umfangreicher (Single-Call ersetzt 2-3 Calls heute, traegt also mehr Output-Felder)
...
- **R-3: Schema-Reduktion-Bedarf.** Wenn p95 doch ueber 5000 Tokens kippt, ADR-83 reduziert (z.B. mentions-Liste optional, weniger Felder).

## Konsequenz fuer ADR-83

**Schema bleibt wie spezifiziert.** Phase 4 (FEAT-03-18) implementiert + misst real. Schema-Reduktion wird Backlog-Item wenn Token-Profil signifikant ueber Schaetzung kippt.
