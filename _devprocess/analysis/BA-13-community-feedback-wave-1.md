# BA-13: Community Feedback Wave 1 (Issues #26 #28 #29 #30, Review-Bot, Dependabot)
...
3. Hartcodierter `.obsidian` Pfad disable in `src/core/knowledge/KnowledgeDB.ts:154`. Hier haben wir die Regel deaktiviert. Mit FEAT-05-07 (konfigurierbarer Agent-Folder, EPIC-05) loesen wir die zugrundeliegende Konstante sauber auf, sodass der Disable nicht mehr noetig ist.
...
|---------|-------|------|----------------|
| OpenAI Tool-Stream | nur `finish_reason === "tool_calls"` flush | post-loop flush fuer alle finish_reasons | FEAT-04-09 (EPIC-04) |
| TMP-Paths | implizite Forward-Slash-Annahme | `normalizePath` + Path-Adapter im FileAdapter | FEAT-18-03 (EPIC-18) |
| Copilot Body | `max_tokens` immer | branch wie OpenAI: `max_completion_tokens` fuer neuere Modelle | FEAT-12-06 (EPIC-12) |
| Agent-Folder | `.obsidian-agent/` hartcodiert | Setting `agentFolderPath`, default unveraendert | FEAT-05-07 (EPIC-05) + ADR-72 |
| Review-Bot | 4 Disable-Klassen + Sentence-Case-Reste | null Disables mit Begruendung, alle Findings sauber | ADR-73 (Querschnitt, IMPL-007 Phase 6) |
| Dependencies | 1 Critical + 2 Medium offen | npm overrides + Smoke-Tests | ADR-74 (Querschnitt, IMPL-007 Phase 1) |
...
- Multi-Vault-Skills-Sharing (Issue #26 erwaehnt das implizit). Erst wenn FEAT-05-07 stabil ist, evaluieren wir Symlink/Submodule-Patterns.
