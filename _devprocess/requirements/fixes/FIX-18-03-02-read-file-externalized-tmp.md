...
(ADR-63). The tool result sent to the model contains a compact reference
...
2. The externaliser (ADR-63) writes the payload via `plugin.globalFs`
   ...
   (ADR-72) and works for absolute paths too.
