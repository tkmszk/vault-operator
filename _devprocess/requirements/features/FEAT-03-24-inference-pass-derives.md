...
priority: P1
effort: M
depends-on: [FEAT-03-15, FEAT-03-17, FEAT-03-18]
related:
...
- Single-Call-Cost-Cap (FEAT-03-18) gilt auch hier -- Inference-Pass-Tokens werden gegen das tagliche Budget gerechnet
...
- **Confidence 0.5-0.85**: Pending-Review-Queue (FEAT-03-18) -- User bestaetigt oder verwirft
...
- **User-Override:** alle derived Facts loeschbar via `delete_fact` (FEAT-03-22)
...
- **Impact:** Job-Scheduler im Plugin-Start (oder setInterval), gemeinsame Cost-Cap-Logik mit FEAT-03-18
...
- LLM muss Tool-Calling-Schema fuer derived Facts unterstuetzen (analog FEAT-03-18)
- Confidence-Bands konsistent mit FactIntegrator (FEAT-03-18) Conflict-Resolution
...
- [ ] Token-Cost-Cap-Schutz (gemeinsam mit FEAT-03-18)
...
- **FEAT-03-15** (Engine-Foundation): facts mit `kind` und `is_latest`, fact_edges mit derived_from_*-Types
- **FEAT-03-17** (Dynamic Composition): derived Facts werden via ContextComposer in Memory-Block aufgenommen
- **FEAT-03-18** (Single-Call Update Pipeline): Token-Cost-Cap, Pending-Review-Queue, FactIntegrator-Logic werden wiederverwendet
