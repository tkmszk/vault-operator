...
priority: P0
effort: M
depends-on: [ADR-79, ADR-78]
related:
  - PLAN-01-memory-v2-master.md (Phase 0.5)
...
> **Hinweis Limitation:** Cascade greift nur fuer `vault://`-URIs (VaultAdapter mit `watch()`-Implementation). Externe Quellen (`file://` ausserhalb Vault, `https://`, `cloud://`) haben kein Watch-Pattern, ihre Refs koennen stale werden. Akzeptierte Limitation, dokumentiert in Adapter-API. Stale-Detection erfolgt lazy on-Resolution-Failure (siehe FEAT-03-17 + ADR-78 Source-Adapter-Registry).
...
- **Impact:** ADR-78 muss vor FEAT-03-15 implementiert sein
...
- [ ] BUG-012 Status auf Resolved aktualisiert mit Verweis auf FEAT-03-14
- [ ] ADR-79 Status auf Accepted
...
- **ADR-79** (Knowledge-DB-Haertung): Architektur-Entscheidung, muss Accepted sein
- **ADR-78** (URI-Versioning): URI-Konvention muss vorher festgelegt sein
...
- **Rename-Cascade fuer externe Source-Schemata** (`file://`, `https://`, `cloud://`): kein FS-Watcher ausserhalb Vault, kein Web-Watch. Stale-Edges werden lazy-detected (FEAT-03-17).
