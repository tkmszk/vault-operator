...
priority: P0
effort: M
depends-on: [FEATURE-0315, FEATURE-0319]
related:
...
Selective Deletion auf vier Granularitaets-Ebenen plus Memory-Eligible-Konzept fuer Conversation-Level-Opt-In. Forget-Right wird ueber Agent-Tools (nicht eigene UI) bedient: User fragt Agent nach Facts und kann sie via Tool-Call loeschen. Das ist konsistent mit FEATURE-0319 (Agent ist das Memory-Interface).
...
- Engine-Public-API: `softDelete`, `hardDelete`, `restoreSoftDeleted`, `runDeletionSweep` muessen exportiert werden (FEATURE-0321)
- Audit-Log darf nicht inflationaer werden (FEATURE-0318 Audit-Pruning gilt: nur state-changing Operations geloggt)

### Open Questions for Architect

- Cascade ueber `mentions_vault_note`: was, wenn die Vault-Note geloescht wird (FEATURE-0314 Cascade)? Werden abgeleitete Facts auch geloescht oder bleiben sie als Reference-Tokens?
...
- [ ] FEATURE-0322 Status: Implemented
...
- **FEATURE-0315** (Engine-Foundation): facts, fact_edges, audit muessen existieren
- **FEATURE-0319** (Living Document UX): Agent-Interface-Konzept etabliert
