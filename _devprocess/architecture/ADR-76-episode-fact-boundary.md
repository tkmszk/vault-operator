...
related:
  ...
  - ADR-18-episodic-task-memory.md
  - ADR-58-semantic-recipe-promotion.md
...
- **`episodes`** (ADR-18): Task-Outcomes mit `mode`, `tool_sequence`, `result_summary`, `success`
- **`recipes`** (ADR-58): Semantische Promotion aus 3+ aehnlichen erfolgreichen Episodes
...
|---|---|---|---|
| `episodes` | Was hat funktioniert (Verhaltens-Outcome) | Append-only, fuettert Recipe-Promotion | Bei jeder erfolgreichen Conversation, automatisch (ADR-18) |
| `recipes` | Wiederverwendbare Verhaltens-Patterns | Promote-on-Threshold, Use-Tracking | Wenn 3+ aehnliche Episodes (ADR-58) |
...
**LLM-Call-Konsolidierung:** Single-Call-Extraction (PLAN-01 Phase 4) produziert in einem Pass:
...
**Supersedes:** ADR-13 (Memory Architecture, 3-Tier-Modell) wird durch ADR-76 + ADR-77 + ADR-13-Update ersetzt. ADR-13 bleibt als historische Referenz.

## Alternatives Considered

1. **Episode-Tabelle abschaffen, alles in Facts** -- Verwirft ADR-18-Investition, Recipe-Promotion muss umgebaut werden, kein Gewinn ausser Schema-Reduktion.
2. **Facts in Episode-Tabelle integrieren** -- Macht Episode-Schema multi-purpose, brechen die ADR-18-Garantien (append-only, success-tracking).
