...
related:
  - ADR-080-persistence-service-pattern.md
  - FEATURE-1404 (MCP Remote Auth)
...
# ADR-081 -- MCP-Tool-Routing + Plugin-Standalone-RPC
...
- **DD-4 Auth-Konsistenz mit FEATURE-1404:** kein paralleler Auth-Mechanismus.
...
**RPC-Authentifizierung:** Bearer-Token + HTTPS (konsistent mit FEATURE-1404). Pro Worker eigenes Token, in `_devprocess/security/safe-storage.json` ueber Electron SafeStorage verschluesselt persistiert.
...
- Konsistent mit FEATURE-1404-Auth-Strategie
...
- **R-1:** OAuth 2.1 (FEATURE-1404 Backlog) wird nicht synchron mit Memory v2 gebaut. **Mitigation:** Bearer-Token reicht fuer MVP, Migration zu OAuth ist additiv.
- **R-2:** RPC-Tool-Schema-Drift zwischen Plugin und Service bei Engine-Update. **Mitigation:** semver-Disziplin (siehe ADR-084), Plugin und Service muessen kompatible Engine-Major-Version haben.
...
- FEATURE-0319 implementiert Plugin-MCP-Routing-Logic + Settings
- FEATURE-1404 bleibt fuer externe-Client-Auth zustaendig (Cloudflare-Tunnel)
