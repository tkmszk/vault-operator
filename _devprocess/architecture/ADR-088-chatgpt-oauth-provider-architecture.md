...
epic: EPIC-021
depends-on: [FEATURE-021-001, FEATURE-021-002, FEATURE-021-003]
---

# ADR-088: ChatGPT OAuth Provider Architecture

**Status:** Accepted (modified by review 2026-04-28)
**Date:** 2026-04-28
...
EPIC-021 fuehrt einen neuen LLM-Provider ein, der das ChatGPT-Plus/Pro-Abo als Backend nutzt. Im Gegensatz zum bestehenden `OpenAiProvider` spricht dieser Provider nicht `api.openai.com`, sondern die undokumentierten Codex-Backend-Endpoints unter `chatgpt.com/backend-api/codex/responses`. Die Authentifizierung folgt einem PKCE-OAuth-Flow gegen `auth.openai.com` mit lokalem Loopback-Callback (siehe ADR-089). Der Token-Lifecycle braucht einen eigenstaendigen Service, weil parallele Refresh-Calls serialisiert werden muessen und der Service spaeter potenziell auch vom Embedding-Pfad genutzt werden koennte.

**Triggering ASRs:**

- Eigener `ChatGptOAuthService` als Singleton (Reliability, Concurrency, FEATURE-021-001).
- Schema-Mapping Codex-Responses zu `ApiStream` (Maintainability, Performance, FEATURE-021-002).
- Tool-Definitions im Responses-Format (Correctness, FEATURE-021-002).
- SafeStorageService-Integration mit verschachteltem Settings-Schema (Security, Maintainability, FEATURE-021-001).
...
- **Separation of Concerns:** Auth-Lifecycle und API-Aufruf sind getrennte Verantwortungen, wie schon bei Copilot (ADR-037).
...
- Con: Verletzt das Pattern, das mit ADR-037 (Copilot) etabliert ist.
...
- Pro: Saubere Trennung, gleiches Pattern wie Copilot (ADR-037).
...
1. Das Copilot-Pattern aus ADR-037 hat sich bewaehrt. Konsistenz ueber Provider hinweg reduziert kognitive Last.
...
ChatGptOAuthService (Singleton, src/core/auth/)
  startAuthFlow()           -> oeffnet Browser, startet Loopback-Server (siehe ADR-089)
...
PkceLoopbackServer (src/core/auth/PkceLoopbackServer.ts, siehe ADR-089)
```
...
- Drei neue Dateien (Provider, Service, Mapper) plus Loopback-Server-Modul (ADR-089).
...
src/core/auth/
  ...
  PkceLoopbackServer.ts        # siehe ADR-089
...
- ADR-011: Multi-Provider API Architecture, erweitert das Adapter-Pattern um einen weiteren `ApiHandler`.
- ADR-019: Electron SafeStorage, Token-Verschluesselung.
- ADR-037: Copilot Provider Architecture, Vorlage fuer das Auth-Service-Pattern.
- ADR-064: Node-fetch-Wrapper, etabliert Node-`https`-Modul als Streaming-Transport.
- ADR-089: PKCE Loopback OAuth Flow (entsteht parallel).

## References

- FEATURE-021-001: ChatGPT OAuth Lifecycle.
- FEATURE-021-002: Codex Responses-API Handler.
- FEATURE-021-003: Settings-UI mit "Mit ChatGPT anmelden".
