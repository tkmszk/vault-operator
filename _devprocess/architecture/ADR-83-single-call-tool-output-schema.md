...
FEAT-03-18 konsolidiert die heutige 2-3-LLM-Call-Pipeline in einen einzigen Single-Call. Das Output-Schema ist der zentrale Vertrag zwischen LLM und FactIntegrator: zu locker -> Parse-Fehler, Drift bei LLM-Provider-Wechsel. Zu strikt -> LLM-Fehlleistungen werden hart abgewiesen.
...
- **DD-4 Engine-Public-API-Stabilitaet:** Schema ist Teil der Engine-Public-API (siehe ADR-84 semver)
...
- `derive`: Edge `derived_from_pattern` zu allen related_fact_ids. Confidence-Banding (siehe ADR-86 fuer Inference-Pass)
...
- **R-2:** Schema-Drift zwischen Engine-Versionen. **Mitigation:** Schema-Version im Output (`schema_version: 1`), Engine-Public-API-Vertrag (ADR-84).
...
- FEAT-03-18 implementiert Schema + Validation in FactIntegrator
- FEAT-03-24 (Inference-Pass) nutzt dasselbe Schema mit `relation: 'derive'`
- FEAT-03-25 (Vault-Note-Source) nutzt dasselbe Schema mit `source_uri`-Feld
- ADR-77 facts-Schema enthaelt `kind`-Spalte und `is_latest`-Spalte

## Open Questions

- Schema-Versioning-Mechanik bei Engine-Major-Bump (ADR-84 deckt das)
