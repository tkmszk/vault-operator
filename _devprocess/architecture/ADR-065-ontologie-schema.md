# ADR-065: Ontologie-Schema und Befuellung
...
EPIC-019 (Knowledge Maintenance) benoetigt transitives Retrieval: "Alles zu Legitimitaet" soll auch Notes ueber Menschenwuerde, Tyrannenmord und Gesellschaftsvertrag finden -- ueber verwandte Konzepte hinweg. Heute kann der Agent nur paarweise Aehnlichkeit erkennen (Cosine-Similarity via ImplicitConnections) und direkte Graph-Nachbarn traversieren (GraphStore BFS).

Es fehlt ein strukturelles Verstaendnis davon, wie Themen und Konzepte zusammenhaengen. Die Ontologie soll diese Luecke schliessen -- als neue Tabelle in der bestehenden KnowledgeDB (ADR-050).

**Triggering ASRs:**
- ASR-5 (FEATURE-1902): Ontologie-Schema -- DB-Design fuer transitive Traversals
- ASR-6 (FEATURE-1902): Ontologie-Befuellung -- aus Vault-Daten vs. LLM-unterstuetzt
...
- **Konsistenz mit EPIC-015**: Muss in die bestehende KnowledgeDB passen (sql.js WASM, ADR-050)
- **Inkrementelle Updates**: Muss bei jedem Ingest (FEATURE-1900) effizient aktualisierbar sein
...
```
1. Ingest-Skill ordnet Note Entitaeten zu (FEATURE-1900)
...
### Risks
- **Cluster-Fragmentierung**: Zu viele kleine Cluster statt weniger grosser Hubs. Mitigation: Lint-Check (FEATURE-1901) erkennt Cluster mit <3 Mitgliedern und schlaegt Merge vor.
...
- ADR-050: SQLite Knowledge DB (Basis-Schema)
- ADR-051: 4-Stufen Retrieval Pipeline (Ontologie als Erweiterung)
- FEATURE-1502: Graph Extraction & Expansion (Datenquelle fuer Bootstrapping)
- FEATURE-1503: Implicit Connection Discovery (Datenquelle fuer Cluster-Hinweise)
