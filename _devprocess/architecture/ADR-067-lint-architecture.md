# ADR-067: Lint-Architektur (Tool, UI und Trigger)
...
FEATURE-1901 (Vault Health Check) soll bei jedem Vault-Open automatisch strukturelle Inkonsistenzen erkennen und als Badge praesentieren. Der Scan selbst basiert auf DB-Queries (0 Token-Kosten), aber die Architektur muss entscheiden: Braucht der Lint ein dediziertes Tool oder reicht der Skill mit bestehenden Tools? Und wie wird das Badge-UI implementiert?

**Triggering ASRs:**
- ASR-3 (FEATURE-1901): Lint als Feature vs. Tool -- Performance
- ASR-4 (FEATURE-1901): Badge-UI + Event-Listener -- Usability
...
- ADR-050: SQLite Knowledge DB (Datenquelle fuer Queries)
- ADR-065: Ontologie-Schema (fuer Cluster-basierte Checks)
- FEATURE-1502: Graph Extraction (edges-Tabelle als Basis)
- FEATURE-1503: Implicit Connections (implicit_edges als Basis)
