...
# ADR-089: ChatGPT PKCE Loopback OAuth Flow
...
- Con: Wenn wir eine eigene Client-ID anlegen wuerden, koennten wir Custom-Scheme registrieren, aber das ist explizit out-of-scope (Codex-Client-ID wird wiederverwendet, ADR-088).
- Con: Funktioniert nicht mit der Codex-Client-ID.

### Option 4: Polling via Device-Code-Flow (analog Copilot)

- Statt Authorization-Code-Flow den Device-Code-Flow nutzen, wie Copilot es macht (siehe ADR-037).
...
- ADR-088: ChatGPT OAuth Provider Architecture, definiert den Service, der den Loopback-Server nutzt.
- ADR-019: Electron SafeStorage, persistiert die Tokens nach erfolgreichem Flow.
