...
related:
  ...
  - FEAT-03-17-dynamic-context-composition.md
  - FEAT-03-18-single-call-update-pipeline.md
triggers:
...
# ADR-82 -- Topic-Inference-Strategie
...
Plus: FEAT-03-18 Single-Call-Extraction generiert bereits Topics fuer neue Facts. Diese sind in `facts.topics` gespeichert. `known_topics`-Tabelle hat `centroid_embedding` (siehe ADR-77).

## Decision Drivers

- **DD-1 Latenz:** Conversation-Start muss < 800ms p95 TTFT bleiben (NFR aus FEAT-03-17)
...
Wenn Drift detected: Topic-Lock wird soft-invalidiert, neue Inference, Topical-Memory-Block fuer naechsten Turn refresh. Drift-Signal wird zudem an FactExtractor (FEAT-03-18) propagiert -- triggert ggf. Re-Extract auch ohne Time-Throttle.
...
- Centroid-Drift-Risiko bei Embedding-Modell-Wechsel (Mitigation: `embedding_model`-Filter siehe FEAT-03-14)
...
- **R-1:** THRESHOLD_TOPIC_MATCH = 0.55 ist Default ohne empirische Daten. **Mitigation:** Eval-Test-Set in FEAT-03-17 misst Topic-Match-Recall, Schwelle wird kalibriert.
- **R-2:** Soft-Lock-Drift-Threshold (Jaccard < 0.4) ist Default. **Mitigation:** Telemetrie (siehe FEAT-03-17 C4) traegt Drift-Events, Schwelle wird in Iteration getunt.
...
- FEAT-03-17 ContextComposer implementiert Topic-Inference
- FEAT-03-17 Reranker-Pass nutzt Topic-Lock fuer Score-Boost (E7)
- FEAT-03-18 FactIntegrator updated Centroids inkrementell beim Insert
- ADR-77 enthaelt `centroid_embedding`-Spalte in `known_topics`
