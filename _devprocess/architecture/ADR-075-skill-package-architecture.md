...
- **Security:** Scripts duerfen nur ueber die Sandbox
  (`evaluate_expression`, ADR-021) laufen. Zip-Importe nur aus
...
- **Review-Bot-Compliance:** Kein `require()`, kein `fetch()`, kein
  `innerHTML`, Pfade ueber `getAgentFolderPath()` (ADR-072).
...
(ueber die bestehende Sandbox, ADR-021). Python/Bash/Shell-Scripts bleiben
...
#### 7. Coordinator-Pattern (FEATURE-2204)
