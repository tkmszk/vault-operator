...
# ADR-80 -- Persistenz-Service-Pattern fuer Memory-v2-Setup-Klassen
...
**Konsequenz:** ADR-80 bleibt valide, aber Implementation wechselt:
...
| (neu) `remote` (RPC zur Service-URL) | **Klasse C Central-Service** | Wird in FEAT-03-19 implementiert. |

**Settings-Migration-Implikation:** Der heutige Wert `storageLocation: 'global'` mappt auf K-A (`persistenceService='local'`, `dbLocation='plugin-local'`). Migration (FEAT-03-23) muss diese Werte beim Plugin-Update transformieren. `local` und `global` werden beide als K-A dargestellt; UI bietet einen vereinfachten Toggle "Plugin-Datenverzeichnis (extern)" vs "Im Vault" (entscheidet zwischen `global`/`local`).

**Wichtige Konsequenz:** MemoryDB ist heute eine Wrapper-Klasse um KnowledgeDB ([src/core/knowledge/MemoryDB.ts:64-70](../../src/core/knowledge/MemoryDB.ts#L64-L70)). Beide nutzen denselben Storage-Layer. Engine-Foundation (FEAT-03-15) muss diese Wrapper-Beziehung erhalten oder durch eigene Engine-Storage-Abstraktion ersetzen.
...
knowledge.db ist orthogonal zur Setup-Klasse: bleibt immer Plugin-bedient (siehe ADR-78, KnowledgeGraphAdapter zweistufig).
...
- Setup-Wechsel zwischen Klassen ist konfigurierbar (FEAT-03-19 MigrationService)
...
- Bei Klasse B (Vault-Sync) bleibt Single-Writer-Lock per PID noetig (siehe ADR-79), weil kein zentraler Service serialisiert
...
- **R-2:** Service-RPC-Multi-Client-Auth ist ein Angriffsvektor. **Mitigation:** Bearer-Token + HTTPS (siehe ADR-81).
...
- FEAT-03-19 (Living Document UX) implementiert die Settings + MCP-Tool-Routing
- FEAT-03-21 (Engine-Extract) frozen die Engine-API als Public-Vertrag
- ADR-79 (Knowledge-DB-Haertung) implementiert Multi-File-Atomic-Commit fuer 1-4 koordiniert geschriebene Files
- ADR-81 (MCP-Tool-Routing + RPC-Auth) konkretisiert die Plugin-Standalone-RPC

## Open Questions

- Service-RPC-Protokoll-Detail (HTTP/JSON-RPC vs MCP-Tunneling): siehe ADR-81
- Service-Failover-Strategie bei Service-Geraet-Crash: post-MVP
- Setup-Wechsel-Migration zwischen K-A/K-B/K-C: FEAT-03-19 MigrationService
