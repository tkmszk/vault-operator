...
The Model Context Protocol (MCP) is a standard for connecting AI agents to external tools and data sources. Obsilo is both client and server. It connects to external MCP servers, and it exposes your vault to external agents like Claude Desktop.
...
On the left, Obsilo reaches out to MCP servers you've configured. A GitHub server that can search issues, a database server that can run queries, whatever tools those servers expose. They become available to the agent alongside its built-in tools.

On the right, Obsilo itself is the server. Claude Desktop connects to it and gets access to your vault for searching notes, reading files, and writing content. Your Obsidian vault becomes a tool that any MCP-compatible agent can use.
...
When Obsilo connects to a server, it discovers available tools and resources through MCP's standard discovery protocol. Those tools appear in the agent's tool list alongside built-in tools. The agent calls them like any other tool and doesn't need to know they run in a separate process.
...
The `get_context` tool is mandatory. External agents should call it first in every conversation. It returns the user profile, memory, behavioral patterns, vault statistics, available skills, and rules, which is the same context Obsilo's internal agent gets from its system prompt.
...
The `search_vault` tool on the MCP server uses the same knowledge layer pipeline described on the [knowledge layer](./knowledge-layer.md) page. External agents get the same 4-stage retrieval (vector search, graph expansion, implicit connections, reranking) as the internal agent. The `write_vault` tool supports batch operations, so create, edit, append, and delete can happen in a single call.
...
You can use Claude Desktop as your primary interface while Obsilo handles the vault integration. Or you can extend Obsilo by connecting it to specialized MCP servers for code analysis, web scraping, or calendar integration. The protocol is the same in both directions.
...
Session sync is marked as mandatory in the tool description. Claude Desktop is instructed to call it at the end of every conversation. In practice it is best-effort. If Claude Desktop terminates the conversation without calling it, the session is simply missing from Obsidian's history.
