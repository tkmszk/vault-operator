...
- **Plugin** — generated by VaultDNAScanner, stored at `.vault-operator/plugin-skills/`. Regenerated automatically when you enable / disable plugins.
- **Built-in** — bundled with the plugin, update with releases.
- **Agent** — created via the skill-creator workflow, quality-gated, carry `source: agent` in frontmatter. Protected from being wiped on plugin reload.
- **User** — manually written, copied, or imported by you. Stored under `.vault-operator/skills/`, persist until you delete them.

All buckets are Markdown files with the same structure. The agent doesn't distinguish between them at runtime. They're all loaded into the system prompt based on relevance to the current conversation. The bucket labels matter for management, not behaviour.
