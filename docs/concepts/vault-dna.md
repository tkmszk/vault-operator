...
Obsidian's plugin ecosystem is one of its strengths. Obsilo can work with your installed plugins, but first it needs to know what's available. VaultDNA handles that automatically.
...
Plugin skills are one category in a broader skill system. Obsilo has three sources of skills: plugin skills (generated by VaultDNAScanner, stored in `.obsidian-agent/plugin-skills/`), user skills (written by you or the agent, stored in `.obsidian-agent/skills/`), and built-in skills (bundled with the plugin).

All three are Markdown files with the same structure. The agent doesn't distinguish between them at runtime. They're all loaded into the system prompt based on relevance to the current conversation. The distinction matters for management: plugin skills are regenerated automatically when plugins change, user skills persist until you delete them, and built-in skills update with plugin releases.
