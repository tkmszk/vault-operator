...
type Holder = { settings: { agentFolderPath?: string } };
...
describe('agentFolder sub-folder helpers (FEAT-29-01)', () => {
    ...
    it('does not break existing helpers (getTmpRoot keeps its semantics for now)', () => {
        const holder: Holder = { settings: { agentFolderPath: '.vault-operator' } };
        // existing tmp helper still returns {root}/tmp during the migration window.
        // After the migration lands, it will be migrated to point under /cache/tmp.
