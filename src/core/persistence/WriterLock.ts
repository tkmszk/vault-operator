 ...
 *
 * Writes a JSON lock file `<dir>/.obsilo-lock` with `{pid, hostname, startedAt}`.
 ...
 * machine. A lock written on Notebook A and seen on Notebook B will look alive
 * by hostname check only. The hostname stamp + Notice in the UI are the
 * mitigation; a hard cross-host serialization needs a real coordination
 * service (Setup-Klasse C).
 *
 ...
 * on the same host already holds the lock. Surfaces `heldBy` so the UI
 * layer can render a Notice.
 */
export class WriterLockHeldError extends Error {
    constructor(public readonly heldBy: LockHolderInfo) {
        super(
            `Knowledge-DB ist gesperrt von ${heldBy.hostname} (PID ${heldBy.pid}). ` +
...
export class WriterLock {
     ...
     * another live PID -- on the same host -- already holds it. Cross-host
     * conflicts cannot be detected here; the caller surfaces them via UI.
     */
    async acquire(): Promise<AcquireResult> {
        ...
        if (existing) {
            ...
            const stale = ageMs > STALE_LOCK_AGE_MS && (!sameHost || !this.isAlive(existing.pid));
            const aliveLocally = sameHost && this.isAlive(existing.pid);

            if (aliveLocally) {
                return { acquired: false, heldBy: existing };
            }
            if (!stale && sameHost) {
                // Same host, same PID still alive but lock fresh -> covered by aliveLocally above.
                // This branch handles "process exited cleanly without releasing": treat as broken.
            }
            // Either remote host (best-effort), dead PID, or stale lock: take it.
    ...
    private async readLock(): Promise<LockHolderInfo | null> {
        try {
            ...
            const parsed = JSON.parse(raw) as Partial<LockHolderInfo>;
            if (
            ...
            return parsed as LockHolderInfo;
        } catch {
            return null;
        }
    }
